...
class CustomQueue():
    ...
    def interrupted(self):
        ...
        return False # Disable interruption checks
...
# Server connections
c = {
    'decision' : {'ip': xf11bm_ws2, 'port': 5551} ,
    'measure' : {'ip': xf11bm_ws1, 'port': 5552} ,
    'analyze' : {'ip': xf11bm_ws2, 'port': 5559} ,
    'analyzeBMM' : {'ip': xf11bm_ws2, 'port': 5555} ,
    }
#c = {
...
#Queue_decision(check_interupted=False)
...
class Queue_decision(CustomQueue): # gpCAM
    ...
    def get(self, save=True, check_interrupted=False, force_load=False, flags:int = 0, use_flags=False):
        return super().get(save=save, check_interrupted=check_interrupted, force_load=force_load, flags=flags, use_flags=use_flags)
...
class Queue_analyzeFixBMM(Queue_analyze): # SciAnalysis

    def __init__(self, from_port=c['measure']['port'], to_port=c['analyzeBMM']['port'], from_ip=c['measure']['ip'], to_ip=c['analyzeBMM']['ip'], name='analyzeBMM', save_dir='./', verbosity=VERBOSITY, **kwargs):
        super().__init__(from_port=from_port, to_port=to_port, from_ip=from_ip, to_ip=to_ip, name=name, save_dir=save_dir, verbosity=verbosity, **kwargs)
