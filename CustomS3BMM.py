...
# import urllib3
# TIMEOUT_SECONDS = 2.0
# http_client = urllib3.PoolManager(timeout=urllib3.Timeout(connect=TIMEOUT_SECONDS, read=TIMEOUT_SECONDS))
...
class CustomS3(Base):
    ...
    def __init__(self, 
        ...
        # Establish timeout:
        # self.client = Minio(endpoint, access_key=username, secret_key=secret_key, http_client=http_client)
...
USERNAME='kyager'
#SECRET_KEY='/home/kyager/BNL/MachineLearning/AutonomousExperiment/beamline-2025_03Mar_03-Karen/key/S3_secret_key.txt'
SECRET_KEY='/nsls2/data/cms/legacy/xf11bm/data/2025_1/KChen-Wiegart6/decision/key/S3_secret_key.txt' # Specify path
#SECRET_KEY=None # Use default path
EXPERIMENT='experiment_AE_20250305BMM'
#EXPERIMENT=None # Default is to use day and hour


class Queue_decision(CustomS3): # gpCAM
    def __init__(self, username=USERNAME, send='decision', receive='analyze', endpoint=ENDPOINT, secret_key=SECRET_KEY, experiment=EXPERIMENT, name='decisionS3BMM', save_dir='./', verbosity=VERBOSITY, **kwargs):
...
class Queue_analyze(CustomS3): # analysis code
    def __init__(self, username=USERNAME, send='analyze', receive='decision', endpoint=ENDPOINT, secret_key=SECRET_KEY, experiment=EXPERIMENT, name='analyzeS3BMM', save_dir='./', verbosity=VERBOSITY, **kwargs):
        super().__init__(username=username, send=send, receive=receive, endpoint=endpoint, secret_key=secret_key, experiment=experiment, name=name, save_dir=save_dir, verbosity=verbosity, **kwargs)
